...
class PermutationListClassificationDataset(Dataset):
    ...
    def __init__(
        ...
        *,
        ...
        starting_vertex: Union[int, Tuple[int, int], List[int]] = -1,  # -1 for random, specific index, or [start, end] range
        seed: int = 42,
    ...
    def _generate(self) -> None:
        for _ in tqdm(range(self.num_samples), desc="Generating permutation dataset samples"):
            ...
            if self.starting_vertex == -1:
                ...
                # Handle list of positions, tuple range, or single index
                if isinstance(self.starting_vertex, list) and len(self.starting_vertex) > 0 and isinstance(self.starting_vertex[0], int):
                    ...
                    if self.starting_vertex < 0 or self.starting_vertex >= self.dag_size:
                        raise ValueError(f"starting_vertex must be -1 (random), list of positions, tuple [start, end], or in range [0, {self.dag_size-1}], got {self.starting_vertex}")
